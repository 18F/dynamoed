...
describe('Acceptance: create, update and delete tables', () => {
  ...
  describe('createTable(name, options)', () => {
    ...
    describe('local indexes', () => {
      xit('packages params correctly with default types', (done) => {
        dynamoed()
          .createTable('users', {
            key: [{id: 'string'}],
            localIndexes: {
          ...
          .then((table) => {
            assert.equal(table.localIndexes[0].type, 'hash')
            assert.equal(table.localIndexes[0].username, 'string')
            assert.equal(table.localIndexes[1].type, 'range')
            assert.equal(table.localIndexes[1].token, 'string')
            done()
          })
          .catch(done)
      })
